...
public class MemoController {
	...
	/*		memoDTO = MemoDTO.builder().m_seq(9999) .m_auth("callor").m_date("2019-12-02").m_time("11시").m_text("댜해나무").build();   */
		...
		memoDTO.setM_date(sdf.format(date));
		memoDTO.setM_time(sdfT.format(date));
		memoDTO.setM_auth(userDTO.getU_id());
		
		
		/* 메모작성을 시작할 때 자동으로 첫 번째 radio를 선택한 상태가 되도록 함 */
		
		List<CatVO> cats = memoService.getCats();
		memoDTO.setM_cat(cats.get(0).getCatValue());
		
		model.addAttribute("CATS", memoService.getCats());
	...
	public String view(@ModelAttribute MemoDTO memoDTO, Model model, @RequestParam("id") String str_seq, HttpSession httpSession) {
	...
	if(userDTO != null && userDTO.getU_id().equals(memoDTO.getM_auth())) {  //로그인한 id와 메모의 m_authㄱ밧을 비교하해 작성자가 아니면 로그인 창으로 건너 뛰기
		model.addAttribute("memoDTO", memoDTO);
		return "view";
	}else {
