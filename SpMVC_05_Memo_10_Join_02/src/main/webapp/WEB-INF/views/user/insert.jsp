...
$(function(){
	...
	//let idcheck = function(){}			// 새로운 함수 선언 방식(함수가 객체)
											// js 함수는 1급 함수다 : 함수 = 객체 = 변수
	
	
		function idcheck(){				// 새창을 열어서 id를 검사하는 코드를 함수로 선언, 표준 함수 선언 방식
		
		
		let top = 500
		//(window.screen.availTop + (window.screen.availHeight / 2)) - (window.screen.height / 2)
	
		let left = 500
		//(window.screen.availLeft + (window.screen.availWidth / 2)) - (window.screen.width / 2)
	...
	let openWin = window.open("${rootPath}/user/idcheck?u_id=" + u_id, "_blank", status)
	...
	$("#id_check").click(idcheck)				// event handler에서 함수목록을 등록하는 절차로 이때는 함수가 마치 객체처럼 등록이 됨(함수 이름에 괄호X)
	...
	<form:form modelAttribute="userDTO"  
		...
		<div class="in-box-border">
			<form:input path="u_id" 
				type="text"
				class="in-box" 
				placeholder="사용자 ID를 입력하고 Enter..."  /><br/>
